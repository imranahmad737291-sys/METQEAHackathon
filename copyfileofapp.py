...
2. Dynamically determine the JOIN path if multiple tables are involved (e.g., Employee → Department → Project).
...
"""
    ...
    return sql_clean
with tab7:
    st.markdown("<div class='section-header'>🗄️ SQL Generator</div>", unsafe_allow_html=True)
    st.markdown("""
    ...
    """, unsafe_allow_html=True)
    schema_file = st.file_uploader("Upload Schema CSV", type=["csv"], key="sql_schema_upload")
    ...
    if schema_file:
        try:
            ...
            if missing:
                st.error("CSV must contain columns: TableName, ColumnName, DataType, Owner")
                schema_df = None
            else:
                st.success(f"Schema loaded: {len(schema_df)} columns from {schema_df[table_col].nunique()} tables.")
                st.dataframe(schema_df)
        except Exception as e:
            st.error(f"Error reading schema CSV: {e}")
